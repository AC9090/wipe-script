...
# Reverted to no USESQL variable GD 15/8/19
...
#
#
#echo "Ran sql access return code " $exitstatus
#sleep 5
#
#
    if [ $exitstatus == 1 ]; then
	if (whiptail --title "$brand" --yesno "Could not update SQL database - do you want to continue the wipe?" 20 78); then
	  echo "Continuing"
	  sleep 2
        else
	  echo "Exiting nwipe"
